...
const NavItem = ({ page, Icon, label, isActive, onClick }) => {
  ...
  return (
    React.createElement("button", {
      ...
      React.createElement(Icon, { className: `w-6 h-6 mb-1 ${isActive ? 'text-primary' : 'text-medium-gray dark:text-dark-gray group-hover:text-primary'}` }),
      React.createElement("span", { className: `text-xs ${isActive ? 'text-primary' : 'text-medium-gray dark:text-dark-gray group-hover:text-primary'}` }, label)
    )
  );
};
...
const BottomNav = () => {
  ...
  return (
    React.createElement("nav", { className: "fixed bottom-0 left-0 right-0 max-w-md mx-auto h-20 bg-card dark:bg-dark-card border-t border-light-gray dark:border-dark-border shadow-[0_-5px_20px_-5px_rgba(0,0,0,0.05)] flex justify-around items-center z-50" },
      ...
      React.createElement("button", {
        ...
        React.createElement(PlusIcon, { className: "w-8 h-8" })
      ),
