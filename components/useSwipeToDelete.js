...
// FIX: The `React` import is required for `React.TouchEvent` and `React.CSSProperties` types.
...
// FIX: Changed to a named export to resolve "Module has no default export" errors.
export const useSwipeToDelete = () => {
  const startX = useRef(0);
  const currentX = useRef(0);
  const elementRef = useRef(null); // Ref to the swipable element
