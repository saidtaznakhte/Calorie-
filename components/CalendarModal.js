...
const CalendarModal = ({ isOpen, onClose, selectedDate, onSelectDate }) => {
  ...
  const handleDayClick = (date) => date && onSelectDate(date);
    ...
    React.createElement("div", {
      ...
      React.createElement("div", {
        ...
        React.createElement("header", { className: "flex items-center justify-between mb-4" },
          React.createElement("button", { onClick: handlePrevMonth, className: "p-2 rounded-full hover:bg-light-gray dark:hover:bg-dark-border", "aria-label": "Previous Month" },
            React.createElement(ChevronLeftIcon, { className: "w-5 h-5 text-text-main dark:text-dark-text-main" })
          ),
          React.createElement("h2", { id: "calendar-modal-title", className: "text-xl font-bold text-text-main dark:text-dark-text-main font-montserrat" }, monthName),
          React.createElement("button", { onClick: handleNextMonth, className: "p-2 rounded-full hover:bg-light-gray dark:hover:bg-dark-border", "aria-label": "Next Month" },
            React.createElement(ChevronRightIcon, { className: "w-5 h-5 text-text-main dark:text-dark-text-main" })
          )
        ),
        React.createElement("div", { className: "grid grid-cols-7 gap-2 text-center text-sm font-semibold text-text-light dark:text-dark-text-light mb-2" },
          daysOfWeek.map(day => React.createElement("span", { key: day }, day))
        ),
        React.createElement("div", { className: "grid grid-cols-7 gap-2 flex-1" },
