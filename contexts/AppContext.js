...
import { Page, PrimaryGoal, ActivityLevel, Gender, UnitSystem } from '../types.js';
...
export const AppProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    notificationIntervalRef.current = window.setInterval(() => {
        ...
        Object.keys(reminders).forEach(key => {
            ...
            if (reminder.enabled && reminder.time === currentTimeStr && lastNotifiedRef.current[reminderType] !== todayStr) {
  ...
  const users = useMemo(() => Object.values(usersData).map(ud => ud.profile), [usersData]);

  const updateCurrentUser = useCallback((updater) => {
    if (!currentUserId) return;
    setUsersData(prev => {
      const currentUserData = prev[currentUserId];
      if (!currentUserData) return prev;
      const updatedUserData = updater(currentUserData);
      return { ...prev, [currentUserId]: updatedUserData };
    });
  }, [currentUserId, setUsersData]);

  const login = useCallback((userId) => {
    if (usersData[userId]) setCurrentUserId(userId);
  ...
  const register = useCallback((profileData, currentWeightLbs) => {
    ...
    const newUser = {
        ...
        waterIntakeHistory: {}, waterGoal: 90,
        stepsHistory: {}, stepsGoal: 10000,
        dayStreak: 0, favoriteFoods: [], preppedMeals: [],
        page: Page.Dashboard, themePreference: 'system',
        customActivities: [], recentFoods: [],
        reminders: {
    ...
    setUsersData(prev => ({ ...prev, [id]: newUser }));
    setCurrentUserId(id);
  ...
  const viewMealDetail = useCallback((index) => {
    ...
    if (index !== null) navigateTo(Page.MealDetail);
  }, [navigateTo]);
  ...
  const handleMealLogged = useCallback((meal) => {
    ...
    showToast({ text: `${meal.name} logged!`, type: 'success' });
  ...
  const handleActivityLogged = useCallback((activity) => {
    ...
    showToast({ text: `${activity.name} logged!`, type: 'success' });
  }, [updateCurrentUser, showToast]);

  const handleMealRemoved = useCallback((mealIndex) => {
    updateCurrentUser(ud => ({ ...ud, loggedMeals: ud.loggedMeals.filter((_, i) => i !== mealIndex) }));
  }, [updateCurrentUser]);

  const handleMealRemovedAndNavigate = useCallback((mealIndex, newPage) => {
    updateCurrentUser(ud => ({ ...ud, loggedMeals: ud.loggedMeals.filter((_, i) => i !== mealIndex), page: newPage }));
    setSelectedMealIndex(null);
  }, [updateCurrentUser]);
  
  const handleActivityRemoved = useCallback((activityIndex) => {
    updateCurrentUser(ud => ({ ...ud, loggedActivities: ud.loggedActivities.filter((_, i) => i !== activityIndex) }));
  }, [updateCurrentUser]);
  ...
  const handleWeightUpdate = useCallback((newCurrentWeight, newGoalWeight) => {
    updateCurrentUser(ud => {
      ...
      if (todayEntryIndex > -1) newHistory[todayEntryIndex] = { date: todayStr, weight: newCurrentWeight };
      else newHistory.push({ date: todayStr, weight: newCurrentWeight });
  ...
  const handleWaterIntakeUpdate = useCallback((date, newIntake) => {
    updateCurrentUser(ud => ({ ...ud, waterIntakeHistory: { ...ud.waterIntakeHistory, [date]: newIntake } }));
    showToast({ text: "Water logged!", type: 'success' });
  }, [updateCurrentUser, showToast]);
  ...
  const addFoodToRecents = useCallback((food) => {
      updateCurrentUser(ud => {
          ...
          return { ...ud, recentFoods: newRecents.slice(0, 5) };
      });
  }, [updateCurrentUser]);
